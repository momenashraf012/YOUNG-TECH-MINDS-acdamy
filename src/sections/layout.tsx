...
export function Eyebrow({ children, center }: { children: ReactNode; center?: boolean }) {
  return (
    <div className={cn('overline mb-3', center ? 'text-center' : 'text-start')}>{children}</div>
  );
}
  ...
  return (
    <div className={cn(center ? 'text-center max-w-[680px] mx-auto' : 'text-start max-w-[620px]')}>
      ...
      <h2
        className={cn(
          font,
          'text-[clamp(28px,3.4vw,38px)] leading-[1.2] text-navy m-0',
          dir === 'rtl' ? 'tracking-normal' : 'tracking-[-0.02em]'
        )}
      >
        {title}
      </h2>
